...
export function DatePickerModal({ initialDate, onSelect, onCancel, heading }: DatePickerModalProps) {
  ...
  const [isClosing, setIsClosing] = useState(false); // NEW state for close animation
  ...
  const { prevValue: prevMonth, currentValue: currentMonth, nextValue: nextMonth } = getPrevNextValues(currentDate, "month")
  const { prevValue: prevYear, currentValue: currentYear, nextValue: nextYear } = getPrevNextValues(currentDate, "year")

  useEffect(() => {
    setIsOpen(true); // open hone par zoom-in
  }, []);

  // ✅ Close handle with animation
  const handleClose = () => {
    setIsClosing(true); // trigger zoom-out
    setTimeout(() => {
      onCancel(); // 300ms baad modal unmount
    }, 300);
  };
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-50 flex items-center justify-center bg-black/55 h-screen transition-opacity duration-300 ease-out
          ${isOpen && !isClosing ? "opacity-100" : "opacity-0"}
        `}
      >
        <div
          className={`bg-[#F2F2F2] p-0 shadow-lg w-[18.75rem] max-w-sm transform transition-all duration-300 ease-out
        ...
        >
          ...
          <div className="grid grid-cols-3 text-center">
            ...
            {/* Up Arrows */}
            <button onClick={() => handleIncrement("day")} className="flex justify-center items-center py-1 text-[#333] hover:text-gray-900">
              <ArrowUp className="w-5 h-5" />
            </button>
            <button onClick={() => handleIncrement("month")} className="flex justify-center items-center py-1 text-[#333] hover:text-gray-900">
              <ArrowUp className="w-5 h-5" />
            </button>
            <button onClick={() => handleIncrement("year")} className="flex justify-center items-center py-1 text-[#333] hover:text-gray-900">
              <ArrowUp className="w-5 h-5" />
            </button>
            ...
            {/* Down Arrows */}
            <button onClick={() => handleDecrement("day")} className="flex justify-center items-center py-1 text-gray-600 hover:text-gray-900">
              <ArrowDown className="w-5 h-5" />
            </button>
            <button onClick={() => handleDecrement("month")} className="flex justify-center items-center py-1 text-gray-600 hover:text-gray-900">
              <ArrowDown className="w-5 h-5" />
            </button>
            <button onClick={() => handleDecrement("year")} className="flex justify-center items-center py-1 text-gray-600 hover:text-gray-900">
              <ArrowDown className="w-5 h-5" />
            </button>
          </div>

          <div className="flex  p-1 w-full space-x-2">
            <button className="mbsc-fr-btn !font-[700] px-4 py-1" onClick={handleClose}>
              CANCEL
            </button>
            <button className="mbsc-fr-btn !font-[700] px-4 py-1" onClick={() => { onSelect(currentDate); handleClose(); }}>
              DONE
            </button>
          </div>
        </div>
